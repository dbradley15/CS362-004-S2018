...
int main() {
	...
	cardEffect(great_hall, choice1, choice2, choice3, &gameState, handpos, &bonus);

	printf("\nIncrease player 1's card count by 1\n");
	assertTrue(gameState.handCount[0], testState.deckCount[0]);

	printf("\nDecrease player 1's deck count by 1");
	assertTrue(gameState.deckCount[0], testState.deckCount[0] - 1);

	printf("\nPlayer 1 gains one action\n");
	assertTrue(gameState.numActions, testState.numActions + 1);
